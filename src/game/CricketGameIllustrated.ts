...
export class CricketGame {
  ...
  private ball!: Mesh;
  private batter!: SpriteCharacter;
  private bowler!: SpriteCharacter;
  private keeper!: SpriteCharacter;
  private fielders: SpriteCharacter[] = [];
  private spriteMaterials = new Map<string, StandardMaterial>();
  ...
  private shotPower = 0;
  private message = '';
  private assetBase = `${import.meta.env.BASE_URL}assets/characters/`;
  ...
  constructor(private canvas: HTMLCanvasElement, private overlay: HTMLDivElement) {
    ...
    this.engine.setHardwareScalingLevel(Math.max(1, Math.min(1.65, window.devicePixelRatio / 1.3)));
    this.scene = new Scene(this.engine);
    this.scene.clearColor = new Color4(0.45, 0.78, 0.95, 1);
    this.camera = new ArcRotateCamera('camera', Math.PI / 2, 1.1, 33, new Vector3(0, 3, 0), this.scene);
  ...
  private spriteMaterial(asset: string): StandardMaterial {
    const cached = this.spriteMaterials.get(asset);
    if (cached) return cached;
    const material = new StandardMaterial(`sprite-${asset}`, this.scene);
    const texture = new Texture(`${this.assetBase}${asset}`, this.scene, true, false);
    ...
    this.spriteMaterials.set(asset, material);
    return material;
  }

  private setSprite(character: SpriteCharacter, asset: string): void {
    character.plane.material = this.spriteMaterial(asset);
  }

  private createSpriteCharacter(
    name: string,
    position: Vector3,
    idleAsset: string,
    width: number,
    ...
    const plane = MeshBuilder.CreatePlane(`${name}-art`, {
      width,
      height,
    ...
    plane.material = this.spriteMaterial(idleAsset);
    plane.isPickable = false;

    const shadow = MeshBuilder.CreateDisc(`${name}-shadow`, { radius: width * 0.31, tessellation: 32 }, this.scene);
    shadow.parent = root;
    shadow.rotation.x = Math.PI / 2;
    shadow.position.y = 0.035;
    shadow.scaling.z = 0.32;
    const shadowMat = this.material(`${name}-shadow-mat`, '#071524');
    shadowMat.alpha = 0.2;
    shadow.material = shadowMat;
    shadow.isPickable = false;

    return { root, plane, idleAsset, actionAsset };
  }

  private buildStadium(): void {
    const grass = MeshBuilder.CreateGround('grass', { width: 68, height: 50 }, this.scene);
    grass.material = this.material('grassMat', '#2c9d55');
    grass.receiveShadows = true;

    const outfieldStripe = MeshBuilder.CreateGround('outfieldStripe', { width: 64, height: 46 }, this.scene);
    outfieldStripe.position.y = 0.01;
    const stripeMat = this.material('outfieldStripeMat', '#36ad60');
    stripeMat.alpha = 0.32;
    outfieldStripe.material = stripeMat;

    const pitch = MeshBuilder.CreateGround('pitch', { width: 7.2, height: 25 }, this.scene);
    pitch.position.y = 0.035;
    pitch.material = this.material('pitchMat', '#caa56c');

    for (const x of [-2.75, 2.75]) {
      const crease = MeshBuilder.CreateBox(`crease-${x}`, { width: 0.07, height: 0.03, depth: 6.5 }, this.scene);
      crease.position.set(x, 0.07, 0);
      crease.material = this.material(`creaseMat-${x}`, '#f7f4e8');
    }

    const boundary = MeshBuilder.CreateTorus('boundary', { diameter: 55, thickness: 0.18, tessellation: 96 }, this.scene);
  ...
  private buildCharacters(): void {
    this.batter = this.createSpriteCharacter('batter', new Vector3(0, 0, -7.5), 'batter-idle.svg', 5.2, 7.2, 1, 'batter-shot.svg');
    this.bowler = this.createSpriteCharacter('bowler', new Vector3(0, 0, 14.5), 'bowler-idle.svg', 4.8, 6.8, 0.93, 'bowler-delivery.svg');
    this.keeper = this.createSpriteCharacter('keeper', new Vector3(0, 0, -11.2), 'keeper.svg', 4.7, 6.5, 0.82);

    for (const [x, z] of [[-14, -2], [14, -2], [-18, 10], [18, 10], [-10, 19], [10, 19]] as const) {
      this.fielders.push(this.createSpriteCharacter(`fielder-${x}-${z}`, new Vector3(x, 0, z), 'fielder.svg', 4.1, 5.7, 0.7));
    }
  ...
  private beginDelivery(): void {
    ...
    this.bowler.root.position.set(0, 0, 18);
    this.setSprite(this.bowler, this.bowler.idleAsset);
    this.ball.position.copyFrom(this.ballOrigin);
    this.renderUI();
  }
  ...
  private animateBat(): void {
    this.setSprite(this.batter, 'batter-shot.svg');
    const start = performance.now();
    const direction = this.swipeDirection.x;
    const tick = () => {
      const p = Math.min(1, (performance.now() - start) / 560);
      const swing = Math.sin(p * Math.PI);
      this.batter.root.rotation.z = -direction * 0.08 * swing;
      this.batter.root.scaling.setAll(1 + swing * 0.045);
  ...
  private animateMiss(): void {
    this.setSprite(this.batter, 'batter-miss.svg');
    const start = performance.now();
    const tick = () => {
      const p = Math.min(1, (performance.now() - start) / 620);
      const sway = Math.sin(p * Math.PI);
      this.batter.root.rotation.z = 0.07 * sway;
      this.batter.root.position.y = -0.12 * sway;
      if (p < 1) requestAnimationFrame(tick);
      else this.resetCharacterPose();
    };
    requestAnimationFrame(tick);
  ...
  private resetCharacterPose(): void {
    ...
    this.setSprite(this.batter, this.batter.idleAsset);
    this.batter.root.rotation.set(0, 0, 0);
    this.batter.root.scaling.setAll(1);
    this.batter.root.position.set(0, 0, -7.5);
  }
  ...
  private update(dt: number): void {
    ...
    if (this.state.delivery === 'idle') this.batter.root.position.y = Math.sin(idle) * 0.025;

    if (this.state.delivery === 'runup') {
      this.bowler.root.position.z -= dt * 8.8;
      const run = Math.sin(this.deliveryTimer * 15);
      this.bowler.root.position.y = Math.abs(run) * 0.1;
      this.bowler.root.rotation.z = run * 0.035;
      if (this.bowler.root.position.z <= 11.7) {
        this.state.delivery = 'released';
        this.setSprite(this.bowler, 'bowler-delivery.svg');
        this.bowler.root.position.y = 0;
        this.bowler.root.rotation.z = 0;
